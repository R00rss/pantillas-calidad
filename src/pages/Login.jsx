...
const Login = () => {
  ...
  function handleLogin(e) {
    ...
    buttonLogin.current.innerHTML = "Iniciando sesión...";
    if (username.current.value === "" || password.current.value === "") {
      ...
      authenticUser(dataUser)
        .then((data) => {
          console.log(data);
          if (data && "auth" in data && data.auth) {
            const paramsAlert = {
              title: "Login",
  ...
  return (
    <section className="bg-black min-h-screen flex justify-center items-center">
      <section className="login-container-animation w-[350px] h-[450px] flex flex-col justify-center items-center bg-gradient-to-tr from-[rgb(42,46,54)] to-[rgb(97,107,125)] rounded-md">
        ...
        <form className="text-slate-50 w-[min(80%,450px)] py-10 flex flex-col text-base">
          ...
          <div className="text-[#000000] font-medium flex justify-center items-center">
            <button
              ...
              className="rounded-full w-[min(150px,95%)] bg-cyan-300 p-[.5rem_.5rem] shadow-2xl border-2 border-transparent hover:border-cyan-300 hover:bg-transparent hover:text-cyan-300 hover:animate-pulse"
              type="button"
            >
              Login
            </button>
